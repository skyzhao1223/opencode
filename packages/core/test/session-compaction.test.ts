...
import { Ref } from "@opencode-ai/core/model"
import { ID as ModelID } from "@opencode-ai/core/model"
import { ID as ProviderID } from "@opencode-ai/core/provider"
...
it.effect("auto compaction bounds the summary request within the context window", () =>
  Effect.gen(function* () {
    ...
    const outcome = yield* compaction.compact({
      ...
      messages,
      model,
      ref: Ref.make({ providerID: ProviderID.make("test-provider"), id: ModelID.make("bound-model") }),
      cost: [],
    })
